...
- GitHub Actions runs the controller matrix on Python 3.13 and the current 3.x, matching the lock's `websockets==17.0` (Python >=3.11); Python 3.10 remains a Core-only compatibility target. The workflow installs pinned controller versions from `plugins/deepscientist-lite-core/controller/requirements.lock` through `tools/validation/install_locked_dependencies.py`; the installer strips artifact-specific hashes only for platform resolution, then verifies every installed version exactly. Release jobs retain full hash verification. Tests use tracked `tests/fixtures/` inputs or fresh temporary directories and must not require ignored `research/.validation-tmp` receipts. Process-resource probes branch by host OS, so Unix CI does not depend on `powershell.exe`.
