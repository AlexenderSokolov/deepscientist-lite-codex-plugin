...
- GitHub Actions installs the pinned controller dependency versions from `plugins/deepscientist-lite-core/controller/requirements.lock` before the unified suite; tests use tracked `tests/fixtures/` inputs or fresh temporary directories and must not require ignored `research/.validation-tmp` receipts. Process-resource probes branch by host OS, so Unix CI does not depend on `powershell.exe`.
