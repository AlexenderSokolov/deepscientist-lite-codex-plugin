...
1. 通过八个 `ds-lite-*` skills 读取项目合同、状态和证据。
...
8. 每次 graph 提交后重建 `RESEARCH_MAP.md`；高频用户入口通过 `mission` / `render-status` 投影到 `STATUS.md`。
9. 使用统一验证脚本执行单元测试、仓库 smoke 和语法检查。
10. 用户文档按“README 快速上手—用户指南理解机制—实现文档维护细节”分层；教学课程用标准库 runner 准备确定性现场。
