...
export PYTHONPATH="$DBOS_DEPENDENCY_ROOT:$ROOT/plugins/deepscientist-lite-control-plane/controller${PYTHONPATH:+:$PYTHONPATH}"
...
"$PYTHON_BIN" "$ROOT/teaching/controller_broker_smoke.py" --codex-bin "$CODEX_BIN" --schema-root "$ROOT/plugins/deepscientist-lite-control-plane/schemas/codex/0.128.0" --workspace "$ROOT" --runtime "$EVIDENCE_ROOT/real-runtime" --output "$EVIDENCE_ROOT/real-fault-broker-smoke.json" --journal-summary "$EVIDENCE_ROOT/broker-journal-summary.json"
