...
& $PythonBin teaching\controller_app_server_smoke.py --codex-bin $CodexBin --home (Join-Path $EvidenceRoot "codex-home") --workspace $Root --schema-root (Join-Path $Root "plugins\deepscientist-lite-core\schemas\codex\0.128.0") --output (Join-Path $EvidenceRoot "canonical-thread-smoke.json")
...
& $PythonBin -c "import ast; from pathlib import Path; files=list(Path(r'$Root\plugins\deepscientist-lite-core\controller\ds_lite_control').glob('*.py')); [ast.parse(p.read_text(encoding='utf-8'), filename=str(p)) for p in files]; print('AST_OK', len(files))"
