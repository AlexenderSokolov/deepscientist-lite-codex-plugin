...
def _cli_identity(
    ...
    *,
    ...
    if expected_cli_version != PINNED_CLI_VERSION:
        raise FreshHostProbeError(f"expected CLI version must be {PINNED_CLI_VERSION}")
    ...
    return {"enforced": True, "expected_version": PINNED_CLI_VERSION, "sha256_match": True}


def _version_observed(lines: list[str]) -> str | None:
    for line in lines:
        match = re.search(r"\bcodex-cli\s+([0-9A-Za-z._-]+)", line, re.IGNORECASE)
        if match:
            return PINNED_CLI_VERSION if match.group(1) == PINNED_CLI_VERSION else "unexpected"
    return None


def _run_command(*, command: list[str], cwd: Path, env: dict[str, str], timeout_seconds: float) -> dict[str, Any]:
    ...
    return {"process_started": started, "returncode_observed": returncode is not None,
            ...
            "version_observed": _version_observed(lines),
...
def run_model_free_checks(*, codex_bin: Path | str, codex_home: Path | str, workspace: Path | str,
    ...
    for label, args in labels:
        result = _run_command(command=prefix + args, cwd=Path(workspace), env=env, timeout_seconds=timeout_seconds)
    ...
    if cli_identity["enforced"]:
        passed = passed and checks[0]["version_observed"] == PINNED_CLI_VERSION
...
def run_once(*, codex_bin: Path | str, codex_home: Path | str, workspace: Path | str,
    ...
    command = ([sys.executable, str(binary)] if binary.suffix.lower() == ".py" else [str(binary)]) + ["exec", "--json", "--sandbox", "read-only", "--skip-git-repo-check"]
    ...
    receipt = {
        ...
        "failure_layer": "none" if cli_task_status == "passed" else ("timeout" if timed_out else "fresh-cli-host"),
        "unverified": [
            "Hook host loading",
