...
CONTRACT_SCHEMA_V2 = "ds-lite.loop-contract.v2"
...
# Backward-compatible alias for tests that patch EXPECTED_CODEX_SHA256
EXPECTED_CODEX_SHA256 = DEFAULT_CODEX_SHA256
EXPECTED_AUTORESEARCH_VERSION = "0.1.5-beta.0"
...
def validate_contract(value: Any) -> dict[str, Any]:
    ...
    if value["schema_version"] not in {CONTRACT_SCHEMA, CONTRACT_SCHEMA_V2} or not ID_RE.fullmatch(str(value["loop_id"])):
    ...
    if value["schema_version"] == CONTRACT_SCHEMA_V2:
        autonomy = value["extensions"].get("autonomy")
        if not isinstance(autonomy, dict) or autonomy != {"retry_policy": "exponential-3", "progress_required": True}:
            raise LoopError("v2 loop contracts require bounded autonomy controls")
    return value


def prepare(args: argparse.Namespace) -> dict[str, Any]:
    goals = _validate_goals(_read_json(Path(args.goals_file)))
    schema_version = getattr(args, "schema_version", CONTRACT_SCHEMA)
    contract = {
        ...
        "extensions": {"autonomy": {"retry_policy": "exponential-3", "progress_required": True}} if schema_version == CONTRACT_SCHEMA_V2 else {},
    }
...
def _validate_autoresearch_binary(path: Path) -> Path:
    """Verify the fixed upstream CLI identity without delegating raw-log storage to it.

    The DS Lite adapter owns process invocation and its redacted result contract;
    the upstream binary is the fixed, inspected compatibility identity.
    """
    try:
        binary = path.resolve(strict=True)
    except OSError as exc:
        raise LoopError("autoresearch_bin must reference an existing fixed binary") from exc
    if not binary.is_file():
        raise LoopError("autoresearch_bin must reference a file")
    try:
        probe = subprocess.run([str(binary), "--version"], cwd=str(binary.parent), capture_output=True,
                               text=True, encoding="utf-8", errors="replace", timeout=15, check=False)
    except (OSError, subprocess.SubprocessError) as exc:
        raise LoopError("autoresearch_bin version probe failed") from exc
    version_text = (probe.stdout or "") + "\n" + (probe.stderr or "")
    if probe.returncode != 0 or not re.search(r"(?<![0-9.])" + re.escape(EXPECTED_AUTORESEARCH_VERSION) + r"(?![0-9.])", version_text):
        raise LoopError("autoresearch_bin version does not match the fixed adapter")
    return binary
...
def _run_process(command: list[str], cwd: Path, timeout: int, input_text: str | None = None) -> dict[str, Any]:
    ...
    if child_state in {"spawn-failed", "pipe-failed"}:
        ...
        failure = "protocol"
    elif returncode == 0 and reduced["terminal_event_observed"] and reduced["result"] is not None:
        # A unique, strict terminal result is stronger than incidental words in
        # tool diagnostics.  This avoids freezing a valid partial turn merely
        # because a preceding tool mentioned an authorization boundary.
        failure = "none"
    elif returncode == 0 and reduced["terminal_event_observed"] and reduced["result"] is not None:
        # A unique, strict terminal result is stronger than incidental words in
        # tool diagnostics.  This avoids freezing a valid partial turn merely
...
def run_loop(args: argparse.Namespace) -> dict[str, Any]:
    ...
    autoresearch: Path | None = None
    if contract["adapter"] in {"native-codex", "codex-autoresearch"}:
        if not args.codex_bin:
            raise LoopError("real adapter requires an explicit pinned --codex-bin")
        codex = _validate_codex_binary(Path(args.codex_bin))
    if contract["adapter"] == "codex-autoresearch":
        if not args.autoresearch_bin:
            raise LoopError("codex-autoresearch requires an explicit fixed --autoresearch-bin")
        autoresearch = _validate_autoresearch_binary(Path(args.autoresearch_bin))
    ...
    for round_index in range(1, contract["budgets"]["max_rounds"] + 1):
        ...
        if contract["adapter"] == "fake":
        ...
        elif contract["adapter"] in {"native-codex", "codex-autoresearch"}:
            ...
            raise LoopError("unsupported loop adapter")
        failure = str(observation.get("failure_layer", "unknown"))
        if contract["adapter"] in {"native-codex", "codex-autoresearch"} and declared == "partial" and not session_id:
            failure = "protocol"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare_parser.add_argument("--schema-version", choices=(CONTRACT_SCHEMA, CONTRACT_SCHEMA_V2), default=CONTRACT_SCHEMA)
