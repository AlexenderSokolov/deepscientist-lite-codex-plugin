...
def _validate_host_mapping(extensions: Any) -> None:
    if not isinstance(extensions, dict):
        raise HandoffError("extensions must be an object")
    host_mapping = extensions.get("host_mapping")
...
def validate_handoff(payload: Any) -> dict[str, Any]:
    ...
    expected = context_digest(
        goal=payload["goal"], facts=payload["observed_facts"], configuration=payload["configuration"]
    )
    if payload["context_digest"] != expected:
        raise HandoffError("context_digest does not match the redacted projection")
    _validate_host_mapping(payload["extensions"])
    _scan(payload)
    return json.loads(json.dumps(payload))


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate a redacted DS Lite handoff.")
    parser.add_argument("command", choices=["validate"])
    parser.add_argument("--path", required=True)
    args = parser.parse_args(argv)
    try:
        payload = json.loads(Path(args.path).read_text(encoding="utf-8"))
        validated = validate_handoff(payload)
    except (OSError, UnicodeError, json.JSONDecodeError, HandoffError) as exc:
        print(json.dumps({"status": "blocked", "error": str(exc)}, ensure_ascii=False))
        return 1
    print(json.dumps({"status": "passed", "schema_version": validated["schema_version"]}))
    return 0
