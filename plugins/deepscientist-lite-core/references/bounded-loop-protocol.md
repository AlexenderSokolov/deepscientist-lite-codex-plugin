...
`failure_layer=none`. The foreground controller may retry a proven-idempotent
provider, network, rate-limit, or timeout failure at most three times with
exponential backoff. Continue independent read-only analysis and offline
...
contract already carries explicit authority. Freeze immediately on publication,
external writes, experiment submission, credential use, `auth`, `protocol`,
`duplicate-risk`, or any non-idempotent operation. A fourth transient failure
is terminal for that gate, but unrelated ready gates continue automatically.
...
- `codex-autoresearch` is an `adopted / adapted` compatibility adapter backed by
  the authorized fixed vendor snapshot and fixed CLI identity. DS Lite owns the
  foreground Codex invocation, redacted result carrier, evidence gate, and
  same-session resume; it therefore does not delegate raw event or runner-log
  storage to the upstream CLI. It requires explicit approved authorization,
  fixed `codex-autoresearch` version, pinned Codex binary, and `--execute`.

No adapter creates a daemon, queue, scheduler, database, MCP service, tmux
capacity, or background process. The controller is a bounded foreground process
that emits a sanitized progress receipt after every terminal action.
