    ...
    for item in registry.get("skills", []):
        ...
        for path in _text_files(runtime_root):
            try:
                ...
                continue
        corpus = "\n".join(text_parts)
        requirement_paths = _requirement_files(runtime_root)
        ...
        manifest_required = (runtime_root / "manifest.yaml").is_file()
        route_complete = all(
            path.is_file()
            for path in (runtime_root / "SKILL.md", runtime_root / "provenance.json")
        ...
        matrix[name] = {
            ...
            "requirements": [path.relative_to(runtime_root).as_posix() for path in requirement_paths],
...
def verify_snapshot(registry: dict) -> dict:
    ...
    for item in registry.get("skills", []):
        ...
        if not runtime_skill.is_file() or not provenance_path.is_file():
        ...
        try:
            ...
            continue
        if f"commit `{registry['upstream']['commit']}`" not in runtime_text:
            mismatches.append(f"{name}:upstream-commit-not-declared")
        if provenance.get("skill") != name or provenance.get("upstream") != item.get("upstream"):
            mismatches.append(f"{name}:provenance-mismatch")
        agent_path = runtime_root / "agents" / "openai.yaml"
        try:
            agent_text = agent_path.read_text(encoding="ascii")
        except (OSError, UnicodeError):
            ...
            if (
                any(field not in normalized_agent_text for field in required_agent_fields)
                or any(field in normalized_agent_text for field in forbidden_agent_fields)
            ):
                mismatches.append(f"{name}:agents/openai.yaml:invalid-stable-contract")
    shared_path = runtime / registry["shared_layer"]["name"]
...
def run_setup(args: argparse.Namespace, *, apply_config: bool) -> dict:
    ...
    if apply_config:
        config = {
            ...
            "source_registry_ref": "plugin://deepscientist-lite-academic/references/nature-skill-registry.json",
        }
...
def verify(args: argparse.Namespace) -> dict:
    ...
    if snapshot["status"] != "passed":
        raise SetupError("academic runtime/provenance verification failed")
