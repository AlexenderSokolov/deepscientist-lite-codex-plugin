...
When editing project files, load [revision-protocol.md](../../references/revision-protocol.md) and validate `ds-lite.revision-constraints.v1` with `scripts/ds_lite_revision_guard.py`. Declare allowed paths, new citations/numbers/theorems, citation or section deletion, approvals, and per-round limits before the edit. Complete one bounded pass and stop at a checkpoint; do not start an unattended rewrite loop.
