...
The same source branch now prepares a 4-case x 3-arm matched-control teaching package. The runner creates equal task/material inputs, staged engineering prompts, runnable math/numerical fixtures, isolated memory treatments, pending result refs, and separate student/instructor guidance. Repository smoke checks preparation only. No 12-arm Codex run, blind score, cost comparison, or effectiveness claim has been completed, so this is not yet `0.5.0-beta.1` release evidence.
