# DeepScientist Lite Codex Plugin 鈥?Research Command Kit

[涓枃璇存槑](README.zh.md) 路 [User Guide](docs/user-guide.zh.md) 路 [Documentation](docs/README.md) 路 [Acknowledgments](ACKNOWLEDGMENTS.md)
...
```mermaid
graph LR
    subgraph "Core 路 0.10.0-beta.2 路 9 skills"
        A["Goal retention 鈫?Route tracing 鈫?Experiment logging 鈫?Evidence packing 鈫?Review 鈫?Iteration 鈫?Delegation 鈫?Handoff"]
    end
    subgraph "Academic 路 0.10.0-beta.2 路 17 skills"
        B["Literature search 路 Citation verification 路 Paper reading 路 Writing 路 Polishing 路 Review simulation 路 Response letters 路 Submission"]
    end
    subgraph "Optional packs 路 0.3.0-alpha.1 路 1 skill each"
        C["Web acquisition"]
...
`intake` sets goals and criteria 鈫?`scout` surveys baselines 鈫?`idea` compares approaches 鈫?`experiment` logs contracts and runs 鈫?`review` checks evidence 鈫?`analysis-write` summarizes findings 鈫?`iterate` reflects and advances.
...
- **Bounded loop adapter (`ds_lite_loop.py`):** Runs consecutive bounded iterations as a supervised loop. Each iteration is a complete "execute 鈫?receipt 鈫?check" cycle with fail-closed external boundaries. Stops on terminal states (completed/blocked/failed/ambiguous).
...
research/
  state/
    graph.json              鈫?Machine-authoritative state graph
  status/
    STATUS.md               鈫?Current node, next step, rollback points (human-readable)
  artifacts/                鈫?What each step did, with public justification
  evidence/<run-id>/        鈫?Experiment contracts, logs, metrics, file hashes
PROJECT.md                  鈫?Project goals and criteria (rarely changes)
RESEARCH_MAP.md             鈫?Research map (rendered from Graph, human-readable)
```
...
- To walk through a guided example: start from the [AI tutorial area](teaching/README.zh.md).
- To browse the plugin source: see [`plugins/deepscientist-lite/`](plugins/deepscientist-lite/).
